...
def optimize_for_web(input_path):
    """
    Optional: If the browser still struggles, this uses ffmpeg
    to re-encode the file into a streamable web format.
    """
    output_path = input_path.replace(".mp4", "_web.mp4")
    try:
        # Command to force H.264 and web-optimized pixel format
        cmd = [
        ...
        print(f"Web optimization skipped (ffmpeg not found): {e}")
...
def process_clip_to_file(clip_data, style, output_path):
    ...
    out = cv2.VideoWriter(output_path, FOURCC, FINAL_FPS, (w, h))
    ...
    for scene in scenes:
        ...
        for frame in clean_frames:
            ...
            # Ensure we write in BGR for OpenCV
    ...
    for idx, temp_file in enumerate(temp_files):
        ...
        while True:
            ...
            if idx > 0 and first_frame_of_clip and last_frame is not None:
                first_frame_of_clip = False  # Only do transition once per clip

                t_type = style.get("transition", "none")
                t_frames = style.get("transition_frames", 10)

                if t_type == "crossfade":
                    trans = crossfade(last_frame, frame_rgb, t_frames)
                elif t_type == "dissolve":
                    trans = dissolve(last_frame, frame_rgb, t_frames)
                elif t_type == "fade_to_black":
                    trans = fade_to_black(last_frame, frame_rgb, t_frames)
                else:
                    trans = []
            ...
            out.write(frame)  # Write the actual frame (already BGR)
            last_frame = frame_rgb

        cap.release()

    out.release()
    time.sleep(0.5)  # Allow OS to release the file
